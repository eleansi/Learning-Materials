...
export function* validateCart(user) {
    ...
    console.info('Here is the response from cart validation', validated);
    return validated;
}
...
export function* validateCard(user) {
    ...
    const { validated } = yield response.json();
    console.info('Here is the response from card validation', validated);
    return validated;
}
...
export function* executePurchase(user) {
    ...
    console.info("Is user charged for the items: ", success);
    return success;
}

export function* checkout() {
    // GET the user and access its properties by calling .get()
    // and choose existing property (user.get('id')) 
    const user = yield select(currentUserSelector);

    console.info("HEY USER WITH ID:", user.get('id'));
    yield put(setCheckoutPhase(QUANTITY_VERIFICATION_CHECKOUT_PHASE));
    const cartValidation = yield call(validateCart, user);
    if(!cartValidation) {
    ...
    const validateCreditCard = yield call(validateCard, user);
